...
export function trySetupGlobalCollectionConfig(provider: CoreProvider, wallet) {
    it("Try create global collection config", async () => {
        try {
            const globalCollectionData =
                await provider.program.account.globalCollectionConfig.fetch(
                    provider.getGlobalCollectionConfig(),
                );
...
export function trySetupBurgerProgramDelegate(
    ...
    it("Try create burger delegate ", async () => {
        try {
            const burgerDelegateData =
                await provider.program.account.programDelegate.fetch(
                    provider.getProgramDelegate(),
                );
...
export function trySetupGameConfig(provider: EpplexProvider, wallet) {
    it("Try create game config ", async () => {
        try {
            const gameConfig = await provider.program.account.gameConfig.fetch(
                provider.getGameConfig(),
            );
        } catch (e) {
...
export function trySetupManagerAccount(provider: WenProvider, wallet) {
